...
angular.module('sortable', []).directive('sortable', sortable);

function sortable() {

  function Controller($attrs, $element, $scope) {
    ...
    ['dragEnd', 'dragMove', 'dragStart'].forEach(function (event) {
      if(angular.isFunction($scope[event])) {
        listeners[event].push($scope[event]);
      }
    });
    ...
    function createDropZone(element) {
      if(angular.isString(options.dropzone)) {
        ...
        dropzone = angular.element(element.prop('tagName'));
      }
    ...
    function onDragEnd(event, child, grandChildren) {
      ...
      if(angular.isArray($scope.ngModel)) {
        ...
        $scope.$apply(function () {
          $scope.ngModel.splice(newIndex, 0, $scope.ngModel.splice(oldIndex, 1)[0]);
        });
      } else {
        dropzone.replaceWith(child);
      }

      listeners.dragEnd.forEach(function (listener) {
        listener(child, newIndex, oldIndex, $scope.ngModel);
      });
    }
    ...
    function onDragMove(event, child, delta) {
      ...
      listeners.dragMove.forEach(function (listener) {
        listener(child, child.data('sortable.index'), $scope.ngModel);
      });
    }
    ...
    function onDragStart(child, grandChildren) {
      ...
      listeners.dragStart.forEach(function (listener) {
        listener(child, child.data('sortable.index'), $scope.ngModel);
      });
    }
  ...
  Controller.$inject = ['$attrs', '$element', '$scope'];
